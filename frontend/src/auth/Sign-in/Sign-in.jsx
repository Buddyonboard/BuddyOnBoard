...
export default function SignIn() {
	...
	async function onSubmit(data) {
		try {
			await firebase.SigninAuthentication(data.email, data.password);
			// console.log('userCredential >', userCredential);

			toast(CONST.loginSuccessfull, {
				position: 'top-right',
				closeButton: true
			});
			setFormError(null);
			...
			<form onSubmit={handleSubmit(onSubmit, onError)} className="space-y-6">
				...
				<div className="grid grid-cols-1 gap-4">
					...
					<ReusableLink
						...
						linkName={CONST.serviceRequestForm.forgotPassword}
					/>
				</div>
				...
				<p className="font-normal text-center text-sm text-bob-link-placeholder-color">
					...
					<Link
						to={CONST.LANDING[0].signupSection.href}
