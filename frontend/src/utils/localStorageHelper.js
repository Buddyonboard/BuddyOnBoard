...
export async function setUserProfileAfterSubmit(
	...
	try {
		...
		// If veriffStatus explicitly passed, use it; otherwise extract from service provider response
		if (veriffStatus) {
			payload.data.veriff.status = veriffStatus;
	...
	localStorage.setItem(
		'veriffStatus',
		JSON.stringify(payload.data?.veriff?.status || null)
	);
	return res;
}
...
/***** Retrieve Veriff Status from LocalStorage ****/
export function getVeriffStatus() {
	const veriffStatus = JSON.parse(localStorage.getItem('veriffStatus'));
	return veriffStatus;
}
